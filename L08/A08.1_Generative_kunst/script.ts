namespace A08_1 {
    ...
    function drawCircles(_position: Vector, _size: Vector): void {
        ...
        let r2min: number = 15;
        let r2max: number = 35;
        let r2: number = Math.floor(Math.random() * (r2max - r2min) + r2min);
    ...
    function drawRectangles(_y: number): void {

        let start: number = 300;
        let widthmin: number = 200;
        let widthmax: number = 300;

        let heightmin: number = 500;
        let heightmax: 100;

        crc2.save();

        let x: number = Math.floor(Math.random() * (widthmax - widthmin) + widthmin);
